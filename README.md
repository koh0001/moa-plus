...
**v1.8.0 신규**: 특수문자 키패드 2페이지화(1페이지 숫자+마침표·쉼표·따옴표, 2페이지 괄호·통화·수학·타이포 기호), 스페이스 드래그 연속 이동(좌우 끝으로 밀고 유지하면 그 방향 자동 이동 + 방향 표시 + 속도 설정).
...
- **자음 대각선 진입 모음** (v1.7) — 자음 키를 ㅣ/ㅡ 대각선 방향으로 긋고 이어 그으면 모음까지 한 번에 완성 (예: ㄱ을 ↗→ = 가, ↙↑ = 고)
