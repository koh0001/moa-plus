...
```
   ...
   [모던]         ← 모던 레이아웃 미니 스크린샷 이미지
   [클래식]       ← 클래식 레이아웃 미니 스크린샷 이미지

   언제든 설정에서 변경 가능
```

- "모던" 선택 → A1 + B2 + C 기본값 (변화 없음)
- "클래식" 선택 → A2 + B1 + C 기본값 + A1 swap 토글 OFF
...
2. **우측 컬럼 (슬롯 A)** — 라디오 (모던 / 클래식) + 토글 (백스페이스 ↔ ㆍ swap, A1 일 때만 활성)
