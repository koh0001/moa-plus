...
> 갱신: 2026-08-14 / 브랜치: **`feat/v2-moakey-parity`** / 미커밋 변경 있음 (아래 §0-0)
> 이 브랜치 = v2.0.0(build 16) 작업 6커밋 (`d4baf27`…`c22a95d`) + 실측 반영 세션(미커밋).
> 베이스는 `feat/symbol-pages-space-scroll`(`501140c`, 입력 반응속도+설정 발견성 — 원격 상태 그대로 보존).
> 테스트: 전체 396건 통과 (serial 실행, `iPhone 17`). §1-2 판독 규칙 준수할 것.
...
### 0순위(완료 직전) — 2026-08-14 실측 반영 세션 결과물 (미커밋)
실기기 체크리스트 결과(a5·b2·b6·T1 꼬리 오타, a6 ㆍ토글 갭, D4 문구)와 재촬영
S1~S3 판독을 반영했다. 변경 요지:
- **엔진**: `reversalThresholdRatio` 0.5→0.70 (반전 등록 = 키폭 28%, adb 실측
  42px/키너비150px 정합. S2 판독 + 진입 길이 불변으로 순정 **절대 임계** 확증 —
  비율 트림 금지, `GestureAnalyzer` 트림 주석 참고).
  구 기본값 0.5 저장분은 디코드 시 0.70 승격. 방향 판정 window 는 등록 임계와
  분리해 키폭 20% 고정 (안 하면 의도적 키폭 50% 되돌림까지 뭉개짐).
  가드: `DeviceMeasuredTailRegressionTests` (13pt 이하 꼬리 = 노이즈,
  18pt = 순정처럼 등록, 25pt 의도적 되돌림 = 유지).
  ㆍ토글 ㅐ↔ㅒ/ㅔ↔ㅖ 추가 (`combineVowels`, a6 종결 — adb 확증).
- **모서리 튕김 흡수 (build 17 실기기 잔여 오타)**: 세로 체인에서 ↓→/↓← 로
  꺾는 모서리의 작은 ↑ 들림이 ↑↓↑→ 로 등록돼 ㅛ 가 ㅘ/ㅕ 를 가로채던 문제.
  `collapseCornerBounces` — 양옆 획의 50% 미만 + 절대 상한(≈21pt)인 중간
  반전/인접 획을 다음 획에 벡터째 흡수 (순정 "≤55° 흡수 + net 재판정" 등가).
  릴리즈 꼬리 ㅛ(b2 "간혹")는 14pt+ 면 순정도 등록하는 크기라 유지 — 더 깎으면
  순정 이탈 + ㅚ 과소인식.
- **D4**: 백스페이스 자소 안내 상시 노출, `복합모음 입력` 피커 라벨 헤더화,
  검색 카탈로그 제목·키워드 보강.
- **신규**: 설정 → 입력 기록 보드(`DebugBoardView`, 실측용 저장 보드),
  긋기 테스트에 획별 각도·길이 카드(T2 대응, `GestureAnalyzer.StrokeInfo`).
- **문서**: `MOAKEY_VIDEO_FINDINGS.md` §C-2 (S1·S2·S3 판독).
- ~~보류: a6 ㆍ토글~~ → adb 실측으로 확증되어 **반영 완료** (아래 1순위 ④).

### 1순위 — adb 정밀 측정 **완료 (2026-08-14)** → 실기기 재검증만 남음
갤럭시 S22+ USB 연결 + adb 터치 주입으로 4개 미확정 항목 전부 종결
(`MOAKEY_VIDEO_FINDINGS.md` §C-3):
①되돌림 하한 42px = 키 너비 28% (→ ratio 0.70 반영) ②ㅓ/ㅣ 경계 22.5° 확증
③첫 획 재해석 45° 옥탄트 확증 (S3 의 60°=수평 관찰은 수기 오차)
④ㆍ토글 ㅐ↔ㅒ/ㅔ↔ㅖ 확증 (→ `combineVowels` 반영, a6 종결).
- **남은 것**: 임계 상향(20%→28%) 후 실기기에서 a5·b2·b6 오타 빈도 재확인.
  기법 재현: `adb shell "input motionevent DOWN x y; ... MOVE ...; UP"` 체인
  (속도 무관·타임아웃 없음이라 느린 주입 OK), 결과는 screencap 크롭 판독.

### 2순위 — 릴리스 진행 결정 (사용자)
- [ ] **PR 생성** — `gh pr create --repo koh0001/moa-plus --base main --head feat/v2-moakey-parity`
      (이 저장소는 포크다 — CI 는 포크 main 대상 PR 에서만 돈다).
      베이스 브랜치(`feat/symbol-pages-space-scroll`)가 main 에 아직 안 들어갔으므로
      **PR 순서**: symbol-pages 먼저 머지 → v2 PR, 또는 v2 PR 하나로 합쳐서(커밋은 이미 분리돼 있음).
- [x] ~~"이번 업데이트" 모달 features 배열 stale~~ — **2026-08-14 갱신 완료**:
      v2.0 내역으로 교체 + 항목이 많아 그룹 헤더(입력 방식/새 도구) 구조로 재편
      (`NewFeaturesModalView`). 앱스토어 릴리스 노트(`07/08_whatsnew`)도 v2.0 으로
      재작성. 신규 설치자 분기(`ContentView.onAppear`)는 현재 코드가 의도된 동작
      (신규 사용자는 first-launch 모달만, lastSeen 선기록으로 What's New 스킵 —
      업데이트 사용자에게는 정상 표시됨을 확인).
...
- [ ] **멀티스트로크 민감도 기본값 0→1** — 영상 판독으로 근거가 생겼다(순정은 속도 거부
      없음 + 반전 임계 키폭 29~33%, E/R5 섹션). 단 실기기 체크리스트 T-1 과 함께 판단할 것.
