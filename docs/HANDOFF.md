...
> 갱신: 2026-08-18 (5차, 햅틱 타이밍) / 브랜치: **`main`** (작업 브랜치 없음)
> **v2.1.1 (build 19) — main 머지 완료. 아카이브/제출 전.**
> PR #21(v2.0.0) · #22(v2.1.0) · #24(v2.1.1) 전부 머지됨. CI 통과 확인.
...
### 상태: v2.1.1 (build 19) — **main 머지 완료, 아카이브 전** (2026-08-18)

`main` 이 곧 릴리스 상태다. 열려 있는 작업 브랜치도, 대기 중인 PR 도 없다.

| 버전 | 내용 | 상태 |
|------|------|------|
| 2.0.0 (17) | 순정 모아키 정합 | **제출 완료, 심사 대기** |
| 2.1.0 (18) | 단축어 개편 (제보 3건) | main 머지, **미제출** |
| 2.1.1 (19) | 햅틱을 누름 시점으로 (이슈 #23) | main 머지, 실기기 확인 완료, **미제출** |

### 1순위 — 지금 해야 할 것

- [ ] **Archive → App Store Connect 제출 (2.1.1)**
      붙여넣을 텍스트는 `docs/appstore/fields/04~08`. **`.md` 정리본을 붙여넣지 말 것** —
      마크다운 기호가 그대로 나가고 줄바꿈이 깨진다 (실제 발생).
      ```bash
      pbcopy < docs/appstore/fields/07_whatsnew_ko.txt
      ```
      2.0.0(17) 이 아직 심사 대기다. 2.1.1 을 올리면 심사 큐에서 그 빌드를 대체한다.
      2.1.x 가 상위 호환이라 대체해도 무방하지만, 2.0 심사 결과를 먼저 받고 싶으면 대기.
- [x] ~~실기기 확인 — 햅틱~~ **완료 (2026-08-18)**. 세 항목 모두 정상:
      누름 진동 1회 / 백스페이스 길게 눌렀을 때 반복 진동 유지 /
      설정 미리보기 키보드 무음
- [ ] **제보자 회신 2건**
      - GitHub 이슈 #23 (mieung) — 이미 CLOSED(자동) 이지만 코멘트는 없다. 반영 내용 한 줄
      - 단축어 제보자(메일) — 초안은 `docs/ABBREVIATION_FEEDBACK_PLAN.md` §7.
        띄어쓰기 트리거를 지원하지 않기로 한 이유와 `ㅋㅋ` 대체안이 들어 있다
- [ ] 심사 리젝 대응 대기. 통과 후: 오타 리포트 채널 모니터링 —
      **입력 기록 보드 리포트**(메일 koh0001@outlook.kr / GitHub 이슈)에 획별 계측·설정
      요약이 담겨 온다. 리포트가 오면 raw/fin 획 수치로
      임계(GestureSettings)·트림(GestureAnalyzer) 재조정 판단.
- [ ] 출시 후 문서 정리 (1.7.2 때의 `dc41599` 패턴): README·사이트 스펙 갱신.

---

## §0-1. 이번 사이클에서 확정된 설계 결정

**되돌리기 전에 이유를 먼저 읽을 것.** 전부 근거가 있고, 대부분은 "풀면 조용히 회귀하는"
종류다. 각 항목은 테스트로 고정돼 있다.

### 햅틱 (v2.1.1, 이슈 #23)
- 햅틱은 **키를 누르는 순간** 울린다. 입력 확정(손 뗄 때)이 아니다 — 긋기 키보드는
  press→release 간격이 길어 확정 시점 진동이 구조적으로 늦게 체감된다
- 그리드 키는 `gestureStarted`, 슬롯B는 `slotBVowelGestureStarted` 가 `keyPressFeedback()` 호출.
  기능행 키는 생성 지점이 22곳이라 `EnvironmentValues.keyPressFeedback` 으로 주입
  (정의는 `KeyboardView.swift` 하단, 기본값 no-op)
- **입력 메서드(`inputConsonant`/`inputSpace`/`toggleLetterMode` …)에 햅틱을 되살리지 말 것**
  — 키 하나에 진동이 두 번 울린다
- 백스페이스만 예외로 `deleteBackward()` 안에 남아 있다. 누른 즉시 한 글자 지우고 자동
  반복으로 이어지는 구조라, 빼면 길게 눌러 연속 삭제할 때 진동이 통째로 사라진다
- 설정 토글은 두지 않았다 (iOS 기본 키보드와 동일 동작)
- 가드: `KeyboardViewModelHapticTimingTests` 6건
...
- 확장 직전 `textBeforeCursor()` 가 트리거로 끝나는지 확인하고 아니면 **확장을 포기**한다.
  델리게이트가 `Bool` 을 반환해 엔진이 되돌리기 상태를 세우지 않는다 — 포기했는데 엔진만
  "확장함" 으로 남으면 사용자가 누른 구분자가 삼켜진다
- 설계 경위: `docs/ABBREVIATION_FEEDBACK_PLAN.md` / 실기기 체크리스트:
  `docs/ABBREVIATION_DEVICE_CHECKLIST.md` / 엔진 규칙 요약: `CLAUDE.md`

### 앱스토어 텍스트 (v2.1.x)
- **제출본은 `docs/appstore/fields/01~08` 뿐이다.** `docs/appstore_description.md` 는 정리본
  + v1.x 릴리스 노트 아카이브이고, 본문은 들어 있지 않다 (붙여넣기 사고 방지용으로 제거)
- 순정 모아키 정합처럼 "계속 유효한 제품 특성" 은 릴리스 노트가 아니라 **앱 설명(05/06)** 에
  둔다. 노트는 버전이 올라가면 통째로 교체된다
- "무엇이 새로운지" 필드에서 **단독 자모(ㅐ ㅢ ㆍ)와 화살표(↗ →)는 써도 된다** —
  v2.0(17) 제출 시 07 원문 그대로 통과했다. 도형문자/통화기호/CJK괄호/em대시는 미검증이라
  계속 배제
...
상세는 `CHANGELOG.md` build 17 섹션과 `MOAKEY_VIDEO_FINDINGS.md` §C-2/C-3 참조.
