...
- [ ] 위 결과에 대한 제품 결정 — 무엇을 어느 릴리스에 넣을지 미정. 특히:
      - 반응속도 1~6·9 번은 워킹트리와 충돌 없음(지금 착수 가능),
        **7·10·12 번은 각각 GestureAnalyzer/FunctionRowView/BackgroundImageManager 와 충돌** →
        현재 미커밋 작업을 커밋한 뒤에 착수할 것.
      - 멀티스트로크 민감도 기본값 0→1 은 **보류 권고**. 성능 수정(1~2번) 후 ㅛ/ㅠ/ㅢ 빠른 입력을
        재측정하고 나서 판단해야 효과 측정이 가능하다.
      - "KeyboardSettings 를 캐싱하자"는 **틀린 방향**(hot path 에 디스크 I/O 없음). 실제 비용은
        구조체 복사·Color 재생성·리렌더 횟수다.
