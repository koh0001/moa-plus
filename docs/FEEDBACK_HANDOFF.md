...
> 2026-06-24 갱신. 이번 세션 커밋 12개(`367095c`~`45841f4`), 작업트리 clean.

### ⚠️ 다음 세션 최우선: ㅣ/ㅡ·ㅛㅠㅕㅑ 입력 디버깅 (미해결 1건)

사용자 피드백 "ㅡㅣ에서 파생 ㅛㅠㅕㅑ 안 됨" 장기 디버깅. **모음 입력 3경로**를 규명(반드시 인지):
1. **전용 ㅣ/ㅡ 키**(모던 col6/col5) — `resolveVowelFromPrimitiveDrag`(누적). ㅣ:↑ㅕ↓ㅑ←ㅓ→ㅏ / ㅡ:↑ㅗ↓ㅜ←ㅛ→ㅠ.
2. **자음 키 대각선** — `VowelResolver`(trie). ↖↗=ㅣ, ↙↘=ㅡ (DiagonalMapping). 자음+모음.
3. **천지인 멀티스트로크** — ↑↓↑=ㅛ, ↓↑↓=ㅠ, ←→←=ㅕ, →←→=ㅑ.

**해결됨(커밋):**
- `057b50c` ㅣ/ㅡ 키 카디널 스냅(forceCardinalOnly) — 기운 긋기 대각선→카디널
- `b667d54` 미리보기 화살표 폭 제한(화면 침범)
- `e1d1919` 긋기 테스트 캔버스가 vowelPrimitive 를 자음 resolver 로 계산하던 표시 버그
- `01115d5`→`0c31c3b` 좁은 키 거리 임계(탭 폴백 방지, keyWidth×0.2 긋기길이 무관)
- `055b26a` **전용 키 천지인 멀티스트로크 매핑 추가**(applySecondaryStroke: ㅚ→ㅛ ㅟ→ㅠ ㅔ→ㅕ ㅐ→ㅑ) + 잘못 넣었던 진폭가드(forceCardinalOnly 0.6) 제거. **사용자 확인: ↓↑↓=ㅠ 됨.**

**❗ 미해결(사용자 답 대기): 확장형 레이아웃 자음 키 대각선 ↙↘=ㅡ, ↖↗=ㅣ 가 실기기에서 안 됨**
- 통합 테스트 `45841f4` 로 **코드 정상 확정**: 확장형 ㄱ↙=그, ㄱ↗=기 통과(기본 4방향off, 매핑 기본값).
- 워크플로 딥 분석: forceCardinalOnly leak 불가(자음 키 매번 false). 막는 건 (a)fourWayMode ON-사용자 꺼짐 (b)대각선 매핑 `.disabled` (c)대각선 섹터 각도 좁음.
- **사용자에게 긋기 테스트로 자음 ㄱ ↙ 화살표 확인 요청함**: ↙인데 ㅡ 안나옴→매핑 disabled / ←↓로 나옴→섹터 각도. **답 대기 중.**
- 다음: 답(화살표)에 따라 설정 가이드 → 둘 다 정상인데 안되면 확장형 특화 코드 버그 추가 조사.

### 🔑 핵심 교훈 (다음 세션 필독)
- **Xcode 클린 빌드 필수** — 키보드 익스텐션은 incremental 빌드에서 코드 변경이 자주 누락. 이번 세션 내내 `xcodebuild clean test` 반복 필요. 사용자 실기기도 `⌘⇧K`(Clean Build Folder)+키보드 삭제/재추가. **스크린샷에 대각선 ↙ 보이면 빌드 미반영 신호.**
- **단위테스트 함정**: directions 를 1개로 주입하면 통과하나, 실기기 촘촘 터치는 2개+ 만들어 다르게 동작. `driveKeyGesture`로 실제 파이프라인 통합 테스트할 것.
- ㅛㅠㅕㅑ는 단일 방향이 아니라 **천지인 멀티스트로크 습관**일 수 있음 — 입력 방식부터 확인.
