...
protocol AbbreviationEngineDelegate: AnyObject {
    ...
    /// Called when a suggestion candidate should be shown
...
final class AbbreviationEngine {
    ...
    private func checkAndExpand(delimiter: Character) {
        // Look up the buffer in the trie
        guard let expansion = lookupTrie(buffer) else {
            // No match - clear buffer and continue
            buffer = ""
            return
        }
        ...
        case .suggestion:
            ...
            delegate?.abbreviationEngine(self, showCandidateFor: expansion)
        }
