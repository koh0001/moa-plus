...
**✅ 폐기(2026-06-24, 사용자 결정): 확장형/클래식 자음 키 대각선 ↙↘=ㅡ, ↖↗=ㅣ 실기기 미작동 → [KNOWN_ISSUES.md](KNOWN_ISSUES.md) KI-2 로 기록 후 종결**
- 크로스레이아웃 단일긋기 통일 작업을 사용자가 보류 → 본 항목 won't-fix.
- 코드 정상(`45841f4` 통합테스트 통과), 의심 원인은 설정(4방향 모드/대각선 `.disabled`/섹터 각도). 상세·재개 진단법은 KI-2 참조.
- ㅣ/ㅡ 필요 시 모던(.vowel) 레이아웃 권장(전용 키).
