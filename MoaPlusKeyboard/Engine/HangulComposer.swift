...
class HangulComposer {
    ...
    private func combineVowels(_ first: Jungseong, _ second: Jungseong) -> Jungseong? {
        ...
        case (.ㅛ, .ㆍ): return .ㅗ
        case (.ㅠ, .ㆍ): return .ㅜ
        // ㅏ↔ㅑ / ㅓ↔ㅕ 도 같은 무한 토글이다 (실기기 제보 2026-08-18).
        // 아래 정방향 4쌍 중 이 둘만 역방향이 빠져 있어서, ㅣㆍㆍ 로 ㅑ 를 만든 뒤
        // ㆍ 를 한 번 더 누르면 ㅏ 로 돌아가지 않고 "ㅑ" 가 커밋되고 ㆍ 가 새로
        // 시작됐다("ㅑㆍ"). **base↔y 6쌍은 반드시 쌍으로 유지할 것** —
        // `test_dotToggle_everyBaseYPairRoundTrips` 가 대칭성을 통째로 고정한다.
        case (.ㅑ, .ㆍ): return .ㅏ
        case (.ㅕ, .ㆍ): return .ㅓ
        ...
        case (.ㅔ, .ㆍ): return .ㅖ
        case (.ㅖ, .ㆍ): return .ㅔ
        // 순정 adb 실측(2026-08-14): 천지인 표준의 마지막-성분 ㆍ 결합 —
        // ㅚ+ㆍ=ㅘ (ㅗ+[ㅣ+ㆍ=ㅏ]), ㅟ+ㆍ=ㅝ (ㅜ+[ㅣ+ㆍ→ㅓ]). 외→와→(+ㅣ)왜,
        // 위→워→(+ㅣ)웨 체인이 열린다. 참고: ㅢ+ㆍ 는 순정에서 옛한글 ㅡㅏ 가
        // 되는 것을 확인 — 현대 한글 밖이라 재현하지 않는다(커밋+ㆍ pending 유지).
        case (.ㅚ, .ㆍ): return .ㅘ
        case (.ㅟ, .ㆍ): return .ㅝ
        default: return nil
        }
