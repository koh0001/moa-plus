...
> 갱신: 2026-08-13 (2차) / 브랜치: **`feat/v2-moakey-parity`** / **origin 푸시 완료**
> 워킹트리 클린. 이 브랜치 = v2.0.0(build 16) 작업 6커밋 (`d4baf27`…`c22a95d`).
> 베이스는 `feat/symbol-pages-space-scroll`(`501140c`, 입력 반응속도+설정 발견성 — 원격 상태 그대로 보존).
> 테스트: 전체 386건 통과 (`** TEST SUCCEEDED **`, `iPhone 17`). §1-2 판독 규칙 준수할 것.
...
### 1순위 — v2.0 실기기 실측 (사람 손가락이 변수인 것들)
**`docs/V2_DEVICE_TEST_CHECKLIST.md`** 를 실기기에서 돌린다. 특히:
- **B1/B2 (회귀 최대 리스크)** — 단독 대각선 ↗/↙ 를 대충·빠르게 그었을 때 손 떼는
  꼬리가 ㅐ/ㅢ/ㅕ 로 승격되는지. 유닛 가드는 있으나 실제 꼬리 분포는 실기기에서만 나온다.
- **A1~A3 (새 경로 성공률)** — 대각선 왕복 ㅐ/ㅔ/ㅢ. 낮으면 T-3(재해석 게이트 0.6 키폭) 조정 판단.
- 실패 기록 형식: `(항목, 의도 글자, 나온 글자, 손 움직임)` — 영상 판독 데이터와 대조 가능.
...
- [ ] 저우선 추가 촬영 S1~S3 (`docs/MOAKEY_RESHOOT_LIST.md`) — 규칙은 확정됐고 수치
      정밀화용. 안 찍어도 출시 지장 없음.
