...
> 문자 규칙 (2026-08-16 갱신 — **실측으로 완화됨**):
> v2.0(build 17) 제출 시 `fields/07_whatsnew_ko.txt` 를 **그대로** 붙여넣어 통과했고,
> 그 파일에는 단독 자모(ㅐ ㅔ ㅢ ㆍ)와 화살표(↗ ↙ → ←)가 들어 있다.
> 즉 **"무엇이 새로운지" 필드에서 자모·화살표는 문제가 되지 않는다.**
> 예시를 말로 풀어 쓰면 오히려 무슨 동작인지 전달이 안 되므로 그대로 쓴다.
>
> 아직 확인 안 된 것(= 굳이 쓰지 말 것): 도형문자, 통화기호, CJK괄호, em대시.
> v1.5 때의 "유효하지 않은 문자" 에러가 이 중 무엇이었는지는 특정되지 않았다.
...
## 한국어 (초안 — 확정본은 fields/07_whatsnew_ko.txt)
단축어를 훨씬 자유롭게 만들 수 있게 정비했습니다. 사용자분들이 보내주신 제보에서 시작한 업데이트입니다.

- 마침표나 물음표 같은 기호가 들어간 단축어를 쓸 수 있습니다. 앞에 붙여도(.ㄱㅅ), 뒤에 붙여도(ㅏ..) 동작합니다.
- 문장에 바로 이어 붙여 써도 인식됩니다. '안녕.ㄱㅅ' 처럼 앞말과 띄어쓰지 않아도 됩니다.
